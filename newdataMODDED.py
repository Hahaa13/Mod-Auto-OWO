...
def c(data,all):
 ...
 if not all == "True":
  menu()
def pm(data,all):
 data['pm'] = input("Toggle Automatically Send Pray/Curse/No (PRAY/CURSE): ")
 if data['pm'].lower()=="pray" or data['pm'].lower()=="curse":
    data['prayid']=input("Do You Want To Pray/Curse A Specified User ? If Yes Enter User ID. Otherwise Enter \"None\": ")
 file = open("settings.json", "w")
 json.dump(data, file)
 file.close()
 print('Successfully saved!')
